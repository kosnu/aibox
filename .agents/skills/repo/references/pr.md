...
3. Read `.github/PULL_REQUEST_TEMPLATE.md` when it exists.
4. Write a concise PR title and body from the actual diff.
5. Link related issues only when verified; omit unverified issue links.
6. Use `closes #<number>` only when the branch satisfies the issue.
7. Push the current branch before PR creation.
8. Run `gh pr create` without `--draft`.
9. Delete any temporary PR body file created during the process.
10. Verify title, body, base branch, head branch, and issue link after creation.
